...
namespace roborts_decision{
    ...
    public:
        ...
        int bullet = 0; //我方机器人子弹数量
        int enemy_bullet = 0; //敌方机器人子弹数量
        int blood = 0; //我方机器人血量
        int enemy_blood = 0;//敌方机器人血量
        const int threat_limit = 777;
        const int threat_difference_limit = 631;
        //int threat = 0; //我方机器人威胁值
        //int enemy_threat = 0; //敌方机器人威胁值
        bool enemy_detect = false; //观测到敌方与否
        bool under_attack = false; //受到攻击与否
      ...
      "/config/costmap_parameter_config_for_decision.prototxt";
        ...
        //获取敌方机器人威胁值
        int get_enemy_threat()
        {
            return enemy_bullet * enemy_blood;
        }
        ...
        bool beyond_threat()
        {
            if(get_threat()>threat_limit)
                return ture;
            return false;
        }

        bool beyond_threat_difference()
        {
            if(abs(get_threat()-get_enemy_threat())>threat_difference_limit)
                return ture;
            return false;
        }
